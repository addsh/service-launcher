...
module "service" {
  ...
  for_each = { for name, service in var.services : name => service if service.compute == "ec2" }
...
module "ecs_service" {
  ...
  for_each = { for name, service in var.services : name => service if service.compute == "ecs" }
